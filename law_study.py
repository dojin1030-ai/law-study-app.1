...
if up:
    ...
    with t1:
        ...
        if not idx_l: st.info("없음")
        else:
            if st.session_state.pos >= len(idx_l): st.session_state.pos = 0
            if st.button("🔄 다음 문제"):
                cur = df.loc[idx_l[st.session_state.pos]].iloc[5]
                if cur in st.session_state.recent: st.session_state.recent.remove(cur)
                st.session_state.recent.append(cur)
                if len(st.session_state.recent) > 3: st.session_state.recent.pop(0)
                cd = [i for i in idx_l if df.loc[i].iloc[5] not in st.session_state.recent]
            ...
            st.caption(f"📍 {' > '.join(pa)}" + (f" ({pr})" if pr else ""))
